...
private:
    ...
    static void encodeInputs(const TTKFileData& fileData, RKGHeader& o_header, QByteArray& o_data);
    static void storeHeader(QDataStream& stream, const RKGHeader& header);

    static bool uncompressInputs(QDataStream& stream, QByteArray& o_uncompressedData, const RKGHeader& header);
    // Todo:
    static void compressInputs(QByteArray& o_toCompress);
...
private:
    ...
    static uint8_t encodeFaceButton(const FrameData& frame, const uint8_t prevMask);
    static uint8_t encodeDirectionInput(const FrameData& frame);
    static uint8_t encodeTrickInput(const FrameData& frame);

    static void loadTail(QDataStream& stream, RKGHeader& o_header);

private:
    static const int RKG_DATASECTION_SIZE = 0x276C;
};
...
protected:
    static uint32_t CRC32(QByteArray& fullData);
};
